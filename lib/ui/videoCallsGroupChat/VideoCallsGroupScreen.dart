...
import 'GroupVideoCallsHandler.dart';
...
class _VideoCallsGroupScreenState extends State<VideoCallsGroupScreen> {
  GroupVideoCallsHandler _signaling;
  ...
  _connect() async {
    ...
    _signaling.onStreamListUpdate = ((streams) async {
      ...
      if (mounted) {
        ...
        await Future.forEach(_listOfStreams, (element) async {
        ...
        _bigRenderer.srcObject = _bigStream;
        setState(() {
          if (!_isCallActive) _isCallActive = streams.isEmpty ? false : true;
        });
      }
    });
    _signaling.initCall(
              ...
              : Column(
                  ...
                  children: <Widget>[
                    ...
                    Padding(
                      ...
                      child: Text(
                        widget.isCaller
                            ? 'Group Video calling ${widget.homeConversationModel.conversationModel.name}'
                            : '${widget.caller.fullName()} is Group Video '
                                'Calling you and ${widget.homeConversationModel.conversationModel.name}',
  ...
  void _deactivate() async {
    ...
    if (_listOfSmallRenderers.isNotEmpty)
      Future.forEach(_listOfSmallRenderers, (element) {
        element.dispose();
      });
  }

  Widget _buildSmallVideoRenderer(
    ...
    return InkWell(
      onTap: () {
        ...
        _bigRenderer.srcObject = _bigStream;
        setState(() {});
//        _listOfSmallRenderers.removeAt(index);
//        _listOfStreams.removeAt(index);
      },
      child: Card(
        ...
        child: Container(
            ...
            child: ClipRRect(
                borderRadius: BorderRadius.circular(25),
                child: RTCVideoView(_listOfSmallRenderers[index], mirror: true,
